...
/**
 * Contenido NATIVO de respaldo de /pedido-recibido (fallback cuando el documento
 * `pedido-recibido` del builder no está publicado). Antes leía la orden de
 * localStorage (`ft_last_order_v1`); ahora trae la orden REAL del backend del
 * tenant igual que OrderConfirmationBlock: lee la referencia de la query string
 * (`id`/`order` → /orders/:id, `number` → /orders/by-number/:number) y consume la
 * API pública. White-label: copy genérico, valores del backend.
 */
 ...
 * `order` (UUID → /orders/:id) y `number` (→ /orders/by-number/:number), las
 * mismas convenciones que usa el storefront (checkout y /pago/retorno?order=). */
...
export default function OrderReceivedNative() {
  ...
  // El retiro en sucursal solo aplica si el tenant tiene sucursales.
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <dd className="font-semibold text-brand-text">
                {isPickup ? "Retiro en sucursal" : order.shippingMethodName || "Envío a domicilio"}
              </dd>
            </div>
          </dl>
        </div>
