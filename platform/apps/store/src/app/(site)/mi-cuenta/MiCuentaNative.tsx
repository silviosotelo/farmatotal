  ...
  const handleSaveDetails = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/customers/${id}`, {
        ...
        body: JSON.stringify({ firstName, lastName, email: detailEmail, phone }),
      });
      if (!res.ok) {
                          ...
                          {orders.map((order) => (
                            <>
                              <tr key={order.id} className="border-t border-[#ededf1]">
                                ...
                                <td className="px-4 py-3 text-brand-muted hidden sm:table-cell">{new Date(order.createdAt).toLocaleDateString(locale)}</td>
