...
export function ProductActions({
  ...
  const add = () => {
    ...
    const item: Product = variant
      ...
      : product;
    addItem(item, qty);
    toast(`${qty} × "${item.title}" agregado al carrito`);
  };
      ...
      {variants.length > 0 && (
        <div className="font-price flex items-baseline gap-2">
          <span className="text-[24px] font-bold leading-none text-brand-orange">
            {formatGs(effPriceWeb)}
          </span>
          {effPriceNormal > effPriceWeb && (
            <span className="text-sm text-price-muted line-through">{formatGs(effPriceNormal)}</span>
          )}
        </div>
      )}
      ...
      {out ? (
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex h-[44px] items-center overflow-hidden rounded-[8px] border border-[#ededf1]">
            <button type="button" aria-label="Disminuir" onClick={() => setQty((q) => Math.max(1, q - 1))} className="focus-ring h-full px-4 text-base text-brand-muted hover:bg-gray-50">−</button>
            <span className="flex h-full min-w-[40px] items-center justify-center border-x border-[#ededf1] px-4 text-sm font-medium tabular-nums" aria-live="polite">{qty}</span>
            <button type="button" aria-label="Aumentar" onClick={() => setQty((q) => Math.min(max, q + 1))} className="focus-ring h-full px-4 text-base text-brand-muted hover:bg-gray-50">+</button>
          </div>
