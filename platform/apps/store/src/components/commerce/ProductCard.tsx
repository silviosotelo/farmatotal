  ...
  if (variant === "compact") {
    return (
      <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
        <Image src={img} alt={name} width={48} height={48} className="rounded object-cover" />
        <div className="min-w-0 flex-1">
          <Link href={`/productos/${product.slug}`} className="text-sm font-medium truncate block hover:text-primary">{name}</Link>
    ...
    <div className="group relative bg-surface rounded-xl border border-border overflow-hidden transition-all hover:shadow-lg hover:-translate-y-0.5" style={{ borderRadius: "var(--radius, 0.5rem)" }}>
      <div className="relative aspect-square overflow-hidden bg-background">
        ...
        <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button className="p-1.5 bg-white/90 rounded-full shadow hover:bg-white" aria-label="Agregar al carrito" onClick={() => addItem(product)}>
      ...
      <div className="p-3">
        <Link href={`/productos/${product.slug}`} className="text-sm font-semibold line-clamp-2 hover:text-primary transition-colors">{name}</Link>
